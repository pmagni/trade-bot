...
class TelegramNotifier:
    ...
    def format_buy_alert(self, symbol: str, price: float, amount: float,
        ...
        details_text = "\n".join(f"  - {html.escape(str(v))}" for v in details.values())
        return (
    ...
    def format_stop_loss_alert(self, symbol: str, entry_price: float,
        ...
        return (
            ...
            f"Perdida: <b>${pnl_usdt:.2f} ({pnl_pct:.1%})</b>\n"
            f"Tipo: {reason}\n\n"
    ...
    def format_scanning_report(self, signals: dict) -> str:
        ...
        for symbol, data in signals.items():
            ...
            above_ema200 = data["indicators"].get("above_ema_200", True)
            trend = "📈 Uptrend" if above_ema200 else "📉 Downtrend"
            buy_gate = 5 if above_ema200 else 6

            buy_bar = "🟩" * min(buy_score, 10) + "⬜" * max(0, 10 - buy_score)
            sell_bar = "🟥" * min(sell_score, 10) + "⬜" * max(0, 10 - sell_score)

            buy_action = data.get("buy_action", "—")
            sell_action = data.get("sell_action", "—")

            lines.append(
                f"<b>{symbol}</b>\n"
                f"${price:,.2f} | RSI: {rsi:.0f} | F&amp;G: {fg}\n"
                f"{trend} (EMA200) | Buy gate: {buy_gate}+\n"
                f"Caida 14d: {drop:.1%} | Subida 14d: {rise:.1%}\n"
                f"Zona: {zone}\n"
                f"Buy:  {buy_bar} ({buy_score}) → {html.escape(str(buy_action))}\n"
                f"Sell: {sell_bar} ({sell_score}) → {html.escape(str(sell_action))}\n"
            )

        status = db.get_state("bot_status", "scanning")
        open_count = len(db.get_open_positions())
        lines.append(f"\nEstado: {status} | Posiciones: {open_count}")
    ...
    async def cmd_signals(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        last_signals = db.get_state("last_signals", "No hay senales recientes")
    ...
    async def cmd_zones(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        zones_text = db.get_state("last_zones", "No hay zonas calculadas aun")
    ...
    async def cmd_config(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_html(
            ...
            f"Reserva minima: {RC.min_reserve_pct:.0%}\n"
    ...
    async def cmd_force_sell(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_html(f"Venta forzada de {symbol} programada para el proximo ciclo.")
    ...
    async def cmd_sizing(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        last = db.get_state("last_sizing", "")
        if last:
            await update.message.reply_html(last)
        else:
            await update.message.reply_html("Sizing no calculado aun. Espera el proximo ciclo de 6h o reinicia el bot.")

    async def cmd_levels(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        from exchange import exchange
        kl = config.key_levels
        lines = ["<b>Key Support &amp; Resistance Levels</b>\n"]
        for symbol, levels in kl.levels.items():
            try:
            ...
            # Resistances (sorted high → low, above price)
            resistances = sorted(levels.get("resistances", []), key=lambda r: r["price"], reverse=True)
            if resistances:
                lines.append("  <b>Resistances:</b>")
                for r in resistances:
                    dist = (price - r["price"]) / r["price"] * 100 if price > 0 else 0
                    lines.append(f"    ⬜ ${r['price']:,.2f} — {r['label']} ({dist:+.2f}%) [+{r['score_bonus']}]")

            lines.append(f"  ─── ${price:,.2f} (current) ───")

            # Supports (sorted high → low, below price)
            supports = sorted(levels.get("supports", []), key=lambda s: s["price"], reverse=True)
            if supports:
                lines.append("  <b>Supports:</b>")
                for s in supports:
                    dist = (price - s["price"]) / s["price"] * 100 if price > 0 else 0
                    lines.append(f"    🟢 ${s['price']:,.2f} — {s['label']} ({dist:+.2f}%) [+{s['score_bonus']}]")

            # Structural stop level
            stop_level = levels.get("stop_level", 0)
            if stop_level > 0:
                lines.append(f"  🛑 Structural stop: ${stop_level:,.2f}")

            lines.append("")
        lines.append(f"Tolerance: {kl.tolerance_pct:.1%}")
        await update.message.reply_html("\n".join(lines))
    ...
    def setup_commands(self, app: Application):
        ...
        app.add_handler(CommandHandler("sizing", self.cmd_sizing))
