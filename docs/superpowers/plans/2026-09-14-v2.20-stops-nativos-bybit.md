...
```bash
scp native_stops.py native_stops_shell.py exchange.py bot.py config.py root@159.223.8.250:/opt/dcabot/
ssh root@159.223.8.250 "systemctl restart swingbot && sleep 8 && systemctl status swingbot --no-pager | head -12"
```

- [ ] **Step 4: Verificar el arranque**

```bash
ssh root@159.223.8.250 "journalctl -u swingbot --since '3 min ago' | grep -iE 'stop nativo|native_stops|error|Scan cycle complete' | tail -20"
```
