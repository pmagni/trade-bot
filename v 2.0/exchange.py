...
class Exchange:
    ...
    def get_orderbook(self, symbol: str, limit: int = 5) -> dict:
        """Get order book."""
        try:
            resp = self.client.get_orderbook(category="spot", symbol=symbol, limit=limit)
            return {
                ...
                for c in account["coin"]:
                    ...
                    if bal > 0:
                        balances[c["coin"]] = {
                            ...
                            "usd_value": float(c["usdValue"]) if c["usdValue"] else 0,
                        }
            return balances
        except Exception as e:
    ...
    def get_min_order_value(self, symbol: str) -> float:
        ...
        try:
            resp = self.client.get_instruments_info(category="spot", symbol=symbol)
    ...
    def place_spot_market_buy(self, symbol: str, quote_amount: float) -> dict:
        ...
        try:
            ...
            # Wait briefly and get fill info
            time.sleep(1)
    ...
    def _get_order_fill(self, symbol: str, order_id: str) -> dict:
        ...
        try:
            resp = self.client.get_order_history(
