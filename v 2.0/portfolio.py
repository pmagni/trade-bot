...
class Portfolio:
    ...
    def get_summary(self) -> Dict:
        ...
        portfolio = self.get_total_value()
        ...
        return {
            "total_value": portfolio["total_usdt"],
    ...
    def save_snapshot(self):
        ...
        try:
            portfolio = self.get_total_value()
            btc_val = portfolio["positions"].get("BTCUSDT", {}).get("value_usdt", 0)
            eth_val = portfolio["positions"].get("ETHUSDT", {}).get("value_usdt", 0)
            open_count = len(db.get_open_positions())

            db.save_snapshot(
                total_value=portfolio["total_usdt"],
                available=portfolio["available_usdt"],
                btc_value=btc_val,
                eth_value=eth_val,
                open_positions=open_count,
            )
            logger.info(f"Snapshot saved: total={portfolio['total_usdt']:.2f} USDT")
        except Exception as e:
